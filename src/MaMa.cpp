    ...
    start:
      ...
      switch (I.v) {
        ...
        case VGet: {
          POP_VALUE(value_v);
          if (value_v.IsObject()) {
            PUSH(value_v.AsObject()->GetProperty(M, I.data.get->name));
            break;
          }
          PUSH_EMPTY();
          break;
        ...
        case VGive: {
          ...
          if (meValue.IsObject()) {
            meValue.AsObject()->SetProperty(M, I.data.give->name, value);
          }
          break;
        }
        case VEq: {
        ...
        case VContains: {
          ...
          if (left.IsObject()) {
            const auto diia_cell =
                left.AsObject()->GetProperty(M, MAG_CONTAINS);
            diia_cell.Retain();
            const auto result = diia_cell.Call(M, {right}, {});
            if (result.IsError()) {
              return result;
        ...
        case VIs: {
          ...
          if (left.IsNumber()) {
            if (right.AsObject() == M->number_structure_object) {
              PUSH_YES();
            } else {
              PUSH_NO();
            }
        ...
        case VNegative: {
          ...
          if (value.IsNumber()) {
            ...
            const auto diia_cell =
                value.AsObject()->GetProperty(M, MAG_NEGATIVE);
            diia_cell.Retain();
        ...
        case VPositive: {
          ...
          if (value.IsNumber()) {
            ...
            const auto diia_cell =
                value.AsObject()->GetProperty(M, MAG_POSITIVE);
            diia_cell.Retain();
            const auto result = diia_cell.Call(M, {}, {});
            if (result.IsError()) {
              return result;
